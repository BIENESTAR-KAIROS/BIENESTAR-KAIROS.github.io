...
const books: Ref<Book[]> = ref(new Array<Book>())
...
onMounted(async () => {
  ...
  await fetch(
    `https://openlibrary.org/search.json?subject=salud mental&limit=12&page=${pageNumber}`,
  )
...
<template>
  ...
  <div v-show="!isLoading">
    <v-container>
      ...
      <v-row>
        <v-col v-for="(book, i) in books" :key="i" cols="6" md="4" lg="3">
          <v-sheet
            rounded="xl"
            height="300"
